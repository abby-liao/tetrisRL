...
logging.basicConfig(level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s',
                    handlers=[logging.FileHandler("training_nintendo_full.log"), logging.StreamHandler()])

os.makedirs("logs/nintendo_full", exist_ok=True)
os.makedirs("models", exist_ok=True)

writer = SummaryWriter("logs/nintendo_full")
recorder = TetrisVideoRecorder(tb_log_dir="logs/nintendo_full", video_trigger=lambda ep: ep % 500 == 0, fps=15)
...
def render_frame(board, episode, score, reward, epsilon, level):
    canvas = np.zeros((450, 400, 3), dtype=np.uint8)
    for r in range(20):
        for c in range(10):
            val = int(board[r, c])
            if val > 0:
    ...
    cv2.putText(canvas, f"Ep: {episode}", (210, 50), font, 0.6, (0, 255, 255), 2)
    cv2.putText(canvas, f"Level: {level}", (210, 90), font, 0.5, (255, 255, 255), 1)
    cv2.putText(canvas, f"Lines: {int(score)}", (210, 130), font, 0.5, (255, 255, 255), 1)
    cv2.putText(canvas, f"Eps: {epsilon:.3f}", (210, 170), font, 0.5, (100, 255, 100), 1)
    return canvas
...
for episode in range(TOTAL_EPISODES):
    ...
    while not done:
        ...
        if episode % 500 == 0:
            frame = render_frame(env.board, episode, total_lines, total_reward, epsilon, current_level)
            recorder.record_frame(frame)
        train_step()
        piece = next_piece

    if episode % 500 == 0:
        recorder.finalize_video(tag="Replay/Full_Metrics", step=episode)
    ...
    if episode % 50 == 0:
        ...
        writer.add_scalar("Features/Aggregate_Height", h, episode)
        writer.add_scalar("Features/Holes", holes, episode)
        writer.add_scalar("Features/Bumpiness", b, episode)
        
        writer.add_scalar("Stats/Pieces_Placed", piece_count, episode)
        writer.add_scalar("Stats/Single_Lines", line_stats[1], episode)
        writer.add_scalar("Stats/Double_Lines", line_stats[2], episode)
        writer.add_scalar("Stats/Triple_Lines", line_stats[3], episode)
        writer.add_scalar("Stats/Tetris_Lines", line_stats[4], episode)
        
        writer.flush()
        logging.info(f"Ep: {episode} | Lines: {total_lines} | Pieces: {piece_count} | Holes: {holes} | Eps: {epsilon:.3f}")

    if episode % 500 == 0:
        torch.save(model.state_dict(), f"models/tetris_full_ep{episode}.pth")
