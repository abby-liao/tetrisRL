...
class TetrisEnv:
    ...
    def get_state_properties(self, board):
        ...
        return np.array([agg_height, 0, holes, bumpiness, row_transitions, col_transitions], dtype=np.float32)
    ...
    def check_collision(self, board, shape, x, y):
        for r, row in enumerate(shape):
            for c, cell in enumerate(row):
                if cell > 0:
                    if (y + r >= self.height or x + c >= self.width or x + c < 0 or board[y + r, x + c] > 0):
                        return True
        return False
